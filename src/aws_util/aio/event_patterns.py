"""Native async event-driven architecture patterns using :mod:`aws_util.aio._engine`.
...
async def transactional_outbox_processor(
...
async def dlq_escalation_chain(
    dlq_url: str,
...
async def _rebuild_aggregate(
    ...
    snap_result = await _snapshot(snapshot_table, aggregate_id, region_name=region_name)
...
async def event_sourcing_store(
    action: str,
    event_store_table: str,
    aggregate_id: str,
    event_type: str = "",
    ...
    if action == "get_events":
        return await _get_events(event_store_table, aggregate_id, region_name)
    ...
    if action == "rebuild":
        ...
        return await _rebuild_aggregate(
            event_store_table,
            snapshot_table,
            aggregate_id,
            region_name,
