"""Native async container_ops utilities using :mod:`aws_util.aio._engine`.
...
async def _get_spot_interruption_rate(
    cluster_name: str,
    period_hours: int,
...
async def _get_placement_success_rate(
    cluster_name: str,
    period_hours: int,
    region_name: str | None,
) -> float:
...
async def _create_placement_failure_alarm(
    cluster_name: str,
    ...
    try:
        await cw.call("PutMetricAlarm", **kwargs)
    ...
    try:
        desc = await cw.call("DescribeAlarms", AlarmNames=[alarm_name])
...
async def ecs_capacity_provider_optimizer(
    cluster_name: str,
    ...
    try:
        ...
        # ------------------------------------------------------------------
        spot_rate = await _get_spot_interruption_rate(
        ...
        # ------------------------------------------------------------------
        placement_rate = await _get_placement_success_rate(
            cluster_name, monitoring_period_hours, region_name
        )
        ...
        # ------------------------------------------------------------------
        alarm_arns = await _create_placement_failure_alarm(
            cluster_name, alarm_sns_topic_arn, region_name
        )
