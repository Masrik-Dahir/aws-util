"""Native async cross-account patterns using :mod:`aws_util.aio._engine`.

Provides async utilities for common cross-account operational patterns
spanning EventBridge, CloudWatch Logs, Kinesis Firehose, S3, DynamoDB,
SQS, and STS.
"""
...
async def cross_account_event_bus_federator(
    ...
    for acct in source_accounts:
        ...
        remote = _remote_client("events", creds, region_name)
        ...
        for idx, pattern in enumerate(patterns):
            ...
            try:
                await asyncio.to_thread(
                    ...
                    State="ENABLED",
                    Description=(
                        f"Forward matching events to central "
                        f"bus {central_bus_name} in "
...
async def centralized_log_aggregator(
    firehose_name: str,
    s3_bucket: str,
    s3_prefix: str,
    ...
    Raises:
        RuntimeError: If any AWS API call fails.
    """
    firehose_c = async_client("firehose", region_name)
    logs_c = async_client("logs", region_name)
    s3_c = async_client("s3", region_name)
    ...
    try:
        ...
        pass  # Fall back to the constructed ARN
...
async def multi_account_resource_inventory(
    ...
    s3_client = async_client("s3", region_name)
