"""networking — VPC connectivity automation utilities.
...
class VPCConnectivityResult(BaseModel):
    """Result of :func:`vpc_connectivity_manager`."""
    ...
    security_group_rules_added: list[str]
    dns_records_created: list[str]
...
def _setup_transit_gateway(
    ...
    try:
        att_resp = ec2.create_transit_gateway_vpc_attachment(
    ...
    # Attach acceptor VPC.
    acc_subnet_ids = _get_vpc_subnets(ec2, acceptor_vpc_id)
    try:
        att_resp2 = ec2.create_transit_gateway_vpc_attachment(
            TransitGatewayId=transit_gateway_id,
    ...
    status = _describe_tgw_attachment_status(ec2, req_att_id)
    return req_att_id, status, routes, sg_rules


def _get_vpc_subnets(ec2: Any, vpc_id: str) -> list[str]:
...
def _find_existing_tgw_attachment(
    ec2: Any,
    tgw_id: str,
    vpc_id: str,
) -> str:
...
def _describe_tgw_attachment_status(
    ec2: Any,
    attachment_id: str,
) -> str:
...
def _setup_privatelink(
    ...
    try:
        svc_resp = ec2.create_vpc_endpoint_service_configuration(
    ...
    consumer_subnets = _get_vpc_subnets(ec2, requestor_vpc_id)
...
def vpc_connectivity_manager(
    ...
    """Automate VPC connectivity patterns.
    ...
    * ``"privatelink"`` — Creates a VPC endpoint service backed by
    ...
    try:
        ...
        if connectivity_type == "peering":
            ...
            connection_id, status, routes, sg_rules = _setup_peering(
                ec2,
                requestor_vpc_id,
                acceptor_vpc_id,
                req_rts,
            ...
            connection_id, status, routes, sg_rules = _setup_transit_gateway(
                ec2,
                transit_gateway_id,
                requestor_vpc_id,
                acceptor_vpc_id,
            ...
            connection_id, status, routes, sg_rules = _setup_privatelink(
                ec2,
                nlb_arn,
                service_name,
                requestor_vpc_id,
        ...
        if hosted_zone_id and dns_record_name:
            ...
            dns_records = _create_dns_record(
                hosted_zone_id,
                dns_record_name,
                target_dns,
                region_name,
