"""ml_pipeline — SageMaker endpoint management and model registry promotion.

Provides high-level functions for deploying SageMaker endpoints with A/B
testing, auto-scaling configuration, and model promotion through the
SageMaker Model Registry with optional cross-account artifact copies.
"""
...
class VariantStatus(BaseModel):
    """Metrics for a single production variant."""
...
class EndpointManagerResult(BaseModel):
    """Result of :func:`sagemaker_endpoint_manager`."""
    ...
    auto_scaling_configured: bool = False
    status: str


class ModelPromotionResult(BaseModel):
    """Result of :func:`model_registry_promoter`."""
    ...
    approval_status: str
    cross_account_copy_location: str | None = None
...
def _build_production_variants(
    variants: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Translate caller-friendly variant dicts to SageMaker API shape."""
    prod_variants: list[dict[str, Any]] = []
    for v in variants:
        prod_variants.append(
            {
                "VariantName": v["model_name"],
                "ModelName": v["model_name"],
                "InstanceType": v["instance_type"],
                "InitialVariantWeight": float(v.get("initial_weight", 1.0)),
                "InitialInstanceCount": int(v.get("initial_instance_count", 1)),
            }
        )
    return prod_variants
...
def _evaluate_winner(
    ...
    """Pick the winning variant based on the evaluation metric.

    Lower is better (errors / latency).  Returns ``None`` when all
    variants have equal metric values or no data.
    """
    if not metrics_map:
        return None
    ...
    for vname, vmetrics in metrics_map.items():
        val = vmetrics.get(evaluation_metric, 0.0)
        if best_val is None or val < best_val:
            best_val = val
            best_name = vname

    # If all values are equal, no clear winner.
    vals = [m.get(evaluation_metric, 0.0) for m in metrics_map.values()]
    if len(set(vals)) <= 1:
        return None
...
def sagemaker_endpoint_manager(
    endpoint_name: str,
    variants: list[dict[str, Any]],
    ...
    """
    try:
        sm = get_client("sagemaker", region_name)
        ...
        endpoint_arn = _create_or_update_endpoint(sm, endpoint_name, config_name, prod_variants)

        # 3. Wait for InService
        status = _wait_for_endpoint(sm, endpoint_name)

        # 4. Configure auto-scaling
        scaling_ok = False
        if auto_scaling_config:
            scaling_ok = _configure_auto_scaling(
                endpoint_name,
                variant_names,
                auto_scaling_config,
                region_name,
        ...
        metrics_map = _collect_variant_metrics(
            endpoint_name,
            variant_names,
...
def _cross_account_copy(
    ...
    s3_src = get_client("s3", region_name)
    try:
...
def model_registry_promoter(
    ...
    try:
        ...
        validation_passed = _validate_model_artifact(model_url, region_name)
        ...
        reg_resp = _register_model_package(
            sm,
        ...
        if cross_account_config and validation_passed:
            cross_account_location = _cross_account_copy(
